...
const getById = async (tenantId, id) => {
  const service = await Service.findOne({ where: { tenantId, id }, include: [variantInclude] });
  if (!service)
    throw Object.assign(new Error("Servicio no encontrado"), { status: 404 });
  return service;
};

const syncVariants = async (tenantId, serviceId, variants = [], basePrice, transaction) => {
  const variantIds = [];
  for (const v of variants) {
    ...
    let variant;
    if (v.id) {
      variant = await ServiceVariant.findOne({ where: { tenantId, id: v.id }, transaction });
    ...
    if (v.modifiers && Array.isArray(v.modifiers)) {
      ...
      for (const m of v.modifiers) {
        ...
        let modifier;
        if (m.id) {
          modifier = await ServiceVariantModifier.findOne({ where: { tenantId, id: m.id }, transaction });
  ...
  await ServiceVariant.destroy({
    where: { tenantId, serviceId, id: { [require("sequelize").Op.notIn]: variantIds } },
    transaction,
  });
};
...
const create = async (tenantId, data) => {
  ...
  const result = await sequelize.transaction(async (t) => {
    const service = await Service.create({ ...serviceData, tenantId }, { transaction: t });
    if (variants && variants.length > 0) {
      await syncVariants(tenantId, service.id, variants, service.price, t);
      ...
      const [baseVariant] = await ServiceVariant.findOrCreate({
        ...
        defaults: { tenantId, serviceId: service.id, price: service.price || 0, durationMinutes: null, images: [], sortOrder: 0, status: 'active' },
        transaction: t,
      })
    ...
    return Service.findOne({ where: { tenantId, id: service.id }, include: [variantInclude], transaction: t });
  });
...
const update = async (tenantId, id, data) => {
  const service = await getById(tenantId, id);
  const { variants, ...serviceData } = data;
  if (!serviceData.slug && serviceData.name)
    serviceData.slug = slugify(serviceData.name);

  if (serviceData.slug && serviceData.slug !== service.slug) {
    const existing = await Service.findOne({ where: { tenantId, slug: serviceData.slug } })
  ...
  const result = await sequelize.transaction(async (t) => {
    ...
    return Service.findOne({ where: { tenantId, id }, include: [variantInclude], transaction: t });
  });
